...
4. Inspect or provision the mailbox first.
5. Reuse the mailbox email address as the sender identity.
6. Verify that toolkit `email` is already published in the room or create the MailBot or equivalent service that will publish it. The normal pattern is a MailBot with `--toolkit-name=email`.
7. Build or update a queue-backed Worker service that consumes the intended queue and uses `--require-toolkit=email` only when the `email` toolkit publisher is real.
8. Validate the YAML or rendered service before deployment.
9. Create or update the service.
10. Verify the live room service appears in `meshagent room service list`.
11. Verify the runtime is actually alive with room developer output, container state, or container logs.
...
15. Before creating the scheduled task, preflight scheduled-task access with `meshagent scheduled-task list --room <ROOM_NAME>` so you know whether scheduler permissions and visibility are actually available for the target room.
