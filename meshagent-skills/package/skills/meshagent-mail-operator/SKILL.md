...
- For MailBot SMTP behavior, inspect the actual implementation in `/src/meshagent-sdk/meshagent-agents/meshagent/agents/mail.py` and `/src/meshagent-sdk/meshagent-agents/meshagent/agents/mail_common.py`.
...
## MailBot SMTP resolution

- MailBot uses `SmtpConfiguration` from `meshagent.agents.mail_common`.
- If `SMTP_USERNAME` is unset, MailBot uses `room.local_participant.get_attribute("name")` as the SMTP username.
- If `SMTP_PASSWORD` is unset, MailBot uses `room.protocol.token` as the SMTP password.
- If `SMTP_HOSTNAME` is unset, MailBot uses the configured mail domain, which defaults to `MESHAGENT_MAIL_DOMAIN` or `mail.meshagent.com`.
- If `SMTP_PORT` is unset, MailBot uses port `587`.
- MailBot reads those values when sending in `MailBot.start_thread` and `MailBot.send_reply_message`. Do not invent a different SMTP retrieval path.
- In a live room workflow, first check whether the default MailBot SMTP values already work before asking the user for manual `SMTP_*` settings.
...
- Do not ask for generic SMTP credentials first if the task is using a MeshAgent MailBot path. Check the MailBot defaults and observed failure mode first.
