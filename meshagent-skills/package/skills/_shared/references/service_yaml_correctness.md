...
- For image-backed webserver release candidates, use `meshagent webserver spec` or an existing valid live `Service` spec as the source manifest. Preserve the generated `ports`, `agents`, and other validated structure, then mutate only the candidate-specific fields such as service name, service id annotation, and container image.
